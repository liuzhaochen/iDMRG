...
function initialize(f!, x0::ITensor, K::Int, buf)
    #aways assume norm(x0) = 1 as start point
    vec = Vector{ITensor}(undef, K)
    alpha = zeros(K)
    beta = zeros(K)
    x0 = copy(x0) / norm(x0)
    ...
    ##################################
    #         MGSIR
    # w, α, β = with_alloc_buffer(buf[2]) do
    #     α = inner(x0, w)
    #     βold = norm(w)
    #     w .+= (-α) .* x0
    #     β = norm(w)
...
function expand!(f!, lan::Lanczos, buf)
...
function lanczos(f, x0::ITensor, LH::ITensor, R::ITensor, buf; tol=1e-10, maxiter=100, verbosity=0, krylovdim=32)
    eng = 0
    err = 0
    [allocate_buffer!(buf[i]) for i in 1:2]
    # x0_b = to_buffer(x0, buf[1])
    x1 = x0
    for i in 1:maxiter
        # L_b = to_buffer(LH, buf[1])
        # R_b = to_buffer(R, buf[1])
        L_b, R_b, x0_b = lanczo_cache_buffer(LH, R, x1, buf[1])
        eng, x1, err = with_alloc_buffer(buf[1]) do
            lanczos_onestep(x -> f(x, L_b, R_b, buf), x0_b; buf, tol, verbosity, krylovdim)
        end
        if err < tol || i == maxiter
            x1 = move_to_heap(x1, buf[1])
            break
        end
        x1 = with_alloc_buffer(buf[2]) do
...
function lanczos_2s(f, x0::ITensor, L::ITensor, R::ITensor, H0::ITensor, H1::ITensor, buf; tol=1e-10, maxiter=100, verbosity=0, krylovdim=32)
    eng = 0
    err = 0
    [allocate_buffer!(buf[i]) for i in 1:2]
    # x0_b = to_buffer(x0, buf[1])
    x1 = x0
    for i in 1:maxiter
        L_b, R_b, H0_b, H1_b, x0_b = lanczo_cache_buffer(L, R, H0, H1, x1, buf[1])
        LH, RH = with_alloc_buffer(buf[2]) do
             L_b*H0_b, R_b*H1_b
        end
        LH_b, RH_b, x0_b = lanczo_cache_buffer(LH, RH, x0_b, buf[1])
        # such that, we only keep one copy of L*R and H*R
        free_buffer!(buf[2])
        allocate_buffer!(buf[2])
        eng, x1, err = with_alloc_buffer(buf[1]) do
            lanczos_onestep(x -> f(x, LH_b, RH_b, buf), x0_b; buf, tol, verbosity, krylovdim)
            # lanczos_onestep(x -> f(x, L_b, R_b, H0_b, H1_b, buf), x0_b; buf, tol, verbosity, krylovdim)
        end
        if err < tol || i == maxiter
            x1 = move_to_heap(x1, buf[1])
            break
...
function lanczos_onestep(f, x0::ITensor; buf, tol=1e-10, verbosity=0, krylovdim=32)
    lan = initialize(f, x0, krylovdim, buf)
    count = 0
    numops = 1
    D = nothing
    U = nothing
